  ...
  <MainLayout page-title="Ativos Observados">
    <div class="observed-assets-page">
      ...
      <div v-else-if="error" class="error-container">
        ...
        <button class="btn btn-primary mt-2" @click="fetchData">Tentar novamente</button>
      </div>
      ...
      <div v-else class="table-container">
        <table class="assets-table">
          ...
          <tbody>
            <tr v-for="asset in paginatedAssets" :key="asset.id" class="asset-row">
              ...
              <td class="text-center">
                <span v-if="asset.alert && asset.alert.valor_min" class="price">R$ {{ formatPrice(asset.alert.valor_min) }}</span>
                <span v-else class="text-muted">-</span>
              </td>
              <td class="text-center">
                <span v-if="asset.alert && asset.alert.valor_max" class="price">R$ {{ formatPrice(asset.alert.valor_max) }}</span>
                <span v-else class="text-muted">-</span>
              </td>
              <td class="text-center">
              ...
              <td class="text-center">
                <div class="action-buttons">
                  <button class="action-btn edit-btn" :title="asset.alert ? 'Editar alerta' : 'Configurar alerta'" @click="openEditModal(asset)">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  ...
                  <button class="action-btn delete-btn" title="Parar de observar" @click="openStopObservingModal(asset)">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
                      <path d="m1 1 22 22" />
                    </svg>
        ...
        <div v-if="totalPages > 1" class="pagination">
          <button :disabled="currentPage === 1" class="pagination-btn" @click="previousPage">
      ...
      <!-- Edit Alert Modal -->
      <EditAlertModal v-if="showEditModal" :asset="selectedAsset" :alert="selectedAsset.alert" @close="closeEditModal" @save="handleSaveAlert" />
      ...
      <div v-if="showStopObservingModal" class="modal-overlay" @click.self="closeStopObservingModal">
        <div class="modal-content small">
          ...
          <div class="modal-body">
            <p>
              Tem certeza que deseja parar de observar o ativo
              <strong>{{ selectedAsset.codigo }}</strong>
              ?
            </p>
            <p class="text-muted">Esta ação irá desativar todos os alertas configurados para este ativo.</p>

            <div class="modal-actions">
              <button type="button" class="btn btn-secondary" @click="closeStopObservingModal">Cancelar</button>
              <button class="btn btn-danger" :disabled="saving" @click="handleStopObserving">
    ...
    computed: {
      ...
      filteredAssets() {
        ...
        if (this.filters.categoria) {
          result = result.filter((asset) => (asset.tipo || asset.categoria) === this.filters.categoria);
        }
