  ...
  <MainLayout page-title="Ativos Observados">
    <div class="observed-assets-page">
      ...
      >
        ...
        <p v-else>
          Adicione ativos à sua carteira e marque como "observando" para
          acompanhar alertas
        </p>
        <router-link
          to="/my-assets"
              ...
              <td class="text-center">
                ...
                >
                  <span class="price-value">
                    R$ {{ formatPrice(asset.currentPrice) }}
                  </span>
                  <span
      ...
      >
        <div class="modal-content small">
          ...
          <div class="modal-body">
            ...
            <p class="text-muted">
              Esta ação irá desativar todos os alertas configurados para este
              ativo.
            </p>
...
export default {
  ...
  computed: {
    categories() {
      const cats = [
        ...new Set(
          this.observedAssets.map(a => a.tipo || a.categoria).filter(Boolean)
        )
      ]
      return cats.sort()
    },
    ...
    filteredAssets() {
      ...
      if (this.filters.categoria) {
        result = result.filter(
          asset => (asset.tipo || asset.categoria) === this.filters.categoria
        )
      }
  ...
  methods: {
    ...
    async handleStopObserving() {
      ...
      try {
        ...
        // Remove from list
        this.observedAssets = this.observedAssets.filter(
          a => a.id !== this.selectedAsset.id
        )
        this.closeStopObservingModal()
      } catch (err) {
        console.error('Erro ao parar de observar:', err)
        alert(
          'Erro ao parar de observar ativo: ' +
    ...
    getCategoryStyle(assetOrCategoria) {
      const asset = typeof assetOrCategoria === 'object' ? assetOrCategoria : null
      const color =
