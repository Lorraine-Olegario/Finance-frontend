<template>
  <MainLayout page-title="Cotações">
    <div class="quotes-page">
      ...
      <div class="page-header">
        ...
        <div class="header-actions">
          ...
          <button class="btn-refresh" :disabled="loading" @click="fetchAllQuotes">
      ...
      <!-- Error -->
      <ErrorAlert v-if="error" type="error" :message="error" dismissible @dismiss="error = ''" />
      ...
      <EmptyState
        ...
        <template #action>
          <button class="btn" @click="fetchAllQuotes">
            <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <polyline points="1 4 1 10 7 10" />
              <polyline points="23 20 23 14 17 14" />
              <path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15" />
    ...
    <!-- Filters Sidebar -->
    <QuotesFilter :is-open="isFilterOpen" :filters="filters" @apply="applyFilters" @close="closeFilters" />
  </MainLayout>
</template>
      ...
      filteredQuotes() {
        ...
        if (this.filters.search) {
          ...
          result = result.filter(
            (q) =>
              q.symbol?.toLowerCase().includes(search) || q.shortName?.toLowerCase().includes(search) || q.longName?.toLowerCase().includes(search),
          );
        }
