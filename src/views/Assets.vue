    ...
    <div class="assets-page">
      ...
      <div class="page-header">
        <div class="header-left">
          <h2>
            <!-- prettier-ignore -->
            <svg class="header-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"> <rect x="2" y="7" width="20" height="14" rx="2" ry="2" /> <path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /> </svg>
            Meus Ativos
          </h2>
            ...
            <tbody>
              ...
              >
                <td class="td-code">
                  <div class="code-cell">
                    <!-- prettier-ignore -->
                    <svg class="icon-asset" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"> <path d="M12 2L2 7l10 5 10-5-10-5z" /> <path d="M2 17l10 5 10-5M2 12l10 5 10-5" /> </svg>
  ...
  computed: {
    filteredAssets() {
      ...
      if (this.filters.categoria) {
        result = result.filter(
          asset => asset.categoria === this.filters.categoria
        )
      }
    ...
    async fetchAssets(page = 1) {
      ...
      try {
        ...
        const ativosPorCategoria =
          response.data?.ativos_por_categoria?.data || {}
        const paginationData =
          response.data?.ativos_por_categoria?.pagination || {}
      ...
      try {
        ...
        if (response.data) {
          if (Array.isArray(response.data.data)) {
            this.categorias = response.data.data.map(
              cat => cat.nome || cat.name || cat
            )
          }
        }
      } catch (error) {
    ...
    async handleDeactivateAsset({ resolve, reject }) {
      try {
        ...
        const errorMsg =
          ...
          error.message ||
          'Erro desconhecido ao desativar o ativo'
    ...
    async handleObserveAsset({ resolve, reject }) {
      try {
        ...
        const errorMsg =
          ...
          error.response?.data?.error ||
          error.message ||
          'Erro ao marcar ativo como observando, tente novamente.'
        reject(new Error(errorMsg))
      }
    },
