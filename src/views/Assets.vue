<template>
  <MainLayout page-title="Gerenciar Ativos">
    <div class="assets-page">
      ...
      <!-- Filters -->
      <AssetsFilter :is-open="filterOpen" :filters="filters" :categorias="categorias" @close="closeFilters" @apply="applyFilters" />
      ...
      <div class="assets-content">
        ...
        <div v-else-if="filteredAssets.length === 0" class="empty-state">
          <svg class="empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg>
        ...
        <div v-else class="assets-table-container">
          <table class="assets-table">
            ...
            <tbody>
              <tr v-for="asset in filteredAssets" :key="asset.id" class="asset-row" :class="{ observing: asset.status === 'observando' }">
                <td class="td-code">
                  <div class="code-cell">
                    ...
                    <div class="asset-code-wrapper">
                      ...
                      <span v-if="asset.status === 'observando'" class="observing-badge" title="Ativo está sendo observado">
                      ...
                      <span v-else class="observing-badge-test" :title="`Status atual: ${asset.status}`">
                ...
                <td class="td-status">
                  <span class="badge badge-status" :class="`status-${asset.status}`">
                    <svg class="status-icon" viewBox="0 0 8 8" fill="currentColor"><circle cx="4" cy="4" r="3" /></svg>
                    {{ asset.status }}
                  </span>
                </td>
                <td class="td-actions">
                  <div class="action-buttons">
                    <button class="action-btn edit-btn" title="Editar ativo" @click="openEditModal(asset)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <button v-if="asset.status === 'ativo'" class="action-btn pause-btn" title="Pausar ativo" @click="openDeactivateModal(asset)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <button v-else class="action-btn play-btn" title="Ativar ativo" @click="openActivateModal(asset)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3" /></svg>
                    </button>

                    <button class="action-btn observe-btn" title="Observar ativo" @click="openObserveModal(asset)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
                        <circle cx="12" cy="12" r="3" />
                      </svg>
                    ...
                    <button class="action-btn delete-btn" title="Deletar ativo" @click="openDeleteModal(asset)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <!-- Modals -->
      <AddAssetModal :is-open="modals.add" :categorias="categorias" :submit-handler="handleAddAssets" @close="closeModal('add')" />
    ...
    computed: {
      filteredAssets() {
        ...
        if (this.filters.search) {
          ...
          result = result.filter((asset) => asset.codigo.toLowerCase().includes(search) || (asset.nome && asset.nome.toLowerCase().includes(search)));
        }
