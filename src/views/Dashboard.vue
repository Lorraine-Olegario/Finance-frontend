...
export default {
  ...
  computed: {
    ...
    topCategories() {
      const categories = Object.entries(this.assetsByType).map(
        ([name, count]) => {
          const percentage =
            this.userAssetsCount > 0
              ? ((count / this.userAssetsCount) * 100).toFixed(1)
              : '0.0'
  ...
  methods: {
    ...
    createChart() {
      ...
      if (
        !this.$refs.assetTypeChart ||
        Object.keys(this.assetsByType).length === 0
      ) {
        return
      }
