...
export function useDashboard() {
  ...
  const categoriesCount = computed(() => {
    return Object.keys(assetsByType.value).filter(key =>
      typeof assetsByType.value[key] === 'number' && assetsByType.value[key] > 0
    ).length
  })
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      await Promise.allSettled([
        assetService.getCategoryColors(userId)
          .then(res => { categoryColors.value = res.data?.colors || {} })
          .catch(() => { categoryColors.value = {} }),

        assetService.getAssetAlerts(userId)
          .then(res => { alertsCount.value = res.data?.length || 0 })
          .catch(() => { alertsCount.value = 0 }),

        authStore.isAdmin && Promise.all([
          userService.getUsers()
            .then(res => { userCount.value = res.data?.length || 0 }),
          assetService.getUserAssets()
            .then(res => { totalAssetsCount.value = res.data?.length || 0 })
        ])
      ])
    } catch (err) {
